...
import RoomGridStudent from "@/components/modules/RoomGridStudent"
...
export default async function Page({ params }: { params: { enrollment_id: string, semester: string, course_id: string, session_id: string } }) {
  ...
  return (
    <div>
      ...
      <RoomGridStudent user_id={session.user.id} session_id={sessionData.session_id} rowCount={sessionData.room.count_row} columnCount={sessionData.room.count_column} attendanceData={attendanceData} />
    </div>
  )
}
