...
export default async function Layout({ params, children }: any) {
  ...
  const items = data.map((item: any) => {
    return(
      <li key={item.session_id} className="border-b p-2 grid grid-cols-5">
        ...
        <Link href={`/dashboard/waterfall/professor/${params.department_id}/courses/${params.course_id}/teaching/${params.teaching_id}/sessions/${item.session_id}/attendance`}>
          Take Attendance
        </Link>
      </li>
    );
  ...
  return (
    <>
      {/* <SessionCreate teaching_id={params.teaching_id} /> */}
