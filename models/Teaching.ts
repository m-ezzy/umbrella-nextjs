...
async function selectTeachingByDegree(degree_id: number) {
  return await queryDatabase(`
    SELECT 
      teaching.id AS teaching_id,teaching.course_id,course.name AS course_name,syllabus_course.semester_number AS semester,teaching.division_id,division.name AS division_name,batch.id AS batch_id,batch.year_started,degree.degree_id,degree.degree_name,teaching.professor_id,user.name_prefix,user.name_first,user.name_sur 
    FROM 
